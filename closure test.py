...
# 2.4 闭包状态下，嵌套函数不可直接修改母函数的标量变量，例如closure part2，foo2函数；当然，可以引用；
# 2.5 闭包状态下，声明nonlocal的情况下，嵌套函数可修改母函数的标量变量，例如closure part3，foo3函数。但如global一样，不推荐使用。推荐闭包中新定义变量，来保存对母函数变量的使用,而非用nonlocal直接修改。
x=1
def foo4():
    y=2
    def bar():
